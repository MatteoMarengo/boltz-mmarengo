...
# Path to the USalign and TMscore binaries. If they’re on your PATH, you can leave them as-is.
USALIGN_CMD = "USalign"
TMSCORE_CMD = "TMscore"
...
def compute_usalign(ref_cif, pred_cif):
    """
    Runs USalign and returns (rmsd: float, aligned_length: int),
    or (None, None) on failure.
    """
    cmd = [USALIGN_CMD, ref_cif, pred_cif]
    try:
    ...
    m = re.search(r"Aligned length=\s*(\d+), RMSD=\s*([\d\.]+)", proc.stdout)
    if not m:
        print("  ⚠️  couldn’t parse USalign output")
        return None, None

    return float(m.group(2)), int(m.group(1))

def compute_tmscore(ref_cif, pred_cif):
    """
    Runs TMscore and returns the first TM-score (float) or None on failure.
    """
    cmd = [TMSCORE_CMD, ref_cif, pred_cif]
    try:
        proc = subprocess.run(cmd, capture_output=True, text=True, check=True)
    except FileNotFoundError:
        print("  ⚠️  TMscore not found (check TMSCORE_CMD or your PATH)")
        return None
    except subprocess.CalledProcessError as e:
        print("  ⚠️  TMscore failed:", e.stderr.strip() or e.stdout.strip())
        return None

    out = proc.stdout
    # Try to grab the first "TM-score = <number>" occurrence
    m = re.search(r"TM-score\s*=\s*([\d\.]+)", out)
    if not m:
        print("  ⚠️  couldn’t parse TMscore output, raw stdout:")
...
def main(main_dir, boltz_csv, output_csv):
    ...
    for row in boltz_rows:
        ...
        us_rmsd, us_atoms = compute_usalign(ref_cif, pred_cif)
        if us_rmsd is not None:
            print(f"  → USalign RMSD = {us_rmsd:.3f} Å over {us_atoms} residues")

        print(f"[{pdb_id}] running TMscore…")
        tm = compute_tmscore(ref_cif, pred_cif)
        if tm is not None:
